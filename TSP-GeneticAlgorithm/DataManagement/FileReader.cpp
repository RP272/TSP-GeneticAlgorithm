...
ConfigurationData FileReader::readConfigurationDataFile()
{
    ...
    if (configurationFile.good()) {
        ...
        std::getline(configurationFile, line);
        result.populationSize = std::stoi(line.substr(line.find(':') + 1));
        std::getline(configurationFile, line);
        result.crossoverRate = std::stof(line.substr(line.find(':') + 1));
        std::getline(configurationFile, line);
        result.mutationRate = std::stof(line.substr(line.find(':') + 1));
        std::getline(configurationFile, line);
